...
## ✨ 功能特性
- 📊 **智能数据库选择**： 找水稻基因去国家水稻中心,RAPDB，找数据下载去NCBI
- 🔍 **智能检索与结果分组**：自动构建 国家水稻中心, NCBI Entrez 检索式，支持通路基因，物种名称、测序策略等条件；按 功能通路或BioProject 自动分组展示，清晰呈现数据来源
- 🎯 **智能过滤**：如目的是重测序则，自动排除 RNA-Seq、Amplicon、ChIP-Seq 等非 WGS 数据
